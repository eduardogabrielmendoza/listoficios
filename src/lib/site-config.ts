...
export const siteConfigSchema = z.object({
  ...
  brand: z.object({
    ...
    compactLogoAssetId: nullableId, faviconAssetId: nullableId, pwaAssetId: nullableId, openGraphAssetId: nullableId,
  }),
  theme: z.object({ preset: z.enum(["forest", "ocean", "graphite"]), brand: z.string().regex(/^#[0-9a-fA-F]{6}$/), ink: z.string().regex(/^#[0-9a-fA-F]{6}$/), accent: z.string().regex(/^#[0-9a-fA-F]{6}$/) }),
  home: z.object({
    eyebrow: z.string().trim().min(3).max(60), title: z.string().trim().min(10).max(90),
    highlight: z.string().trim().min(2).max(45), description: z.string().trim().min(30).max(240),
    primaryCtaLabel: z.string().trim().min(2).max(40), primaryCtaHref: internalHref,
    professionalCtaLabel: z.string().trim().min(2).max(40), professionalCtaHref: internalHref,
    featuredProfileSlug: z.string().trim().max(120), heroAssetId: nullableId,
    trustTitle: z.string().trim().min(5).max(90), trustDescription: z.string().trim().min(20).max(220),
    finalCtaTitle: z.string().trim().min(5).max(90), finalCtaDescription: z.string().trim().min(20).max(200),
  }),
  motion: homeMotionSchema,
  footer: z.object({ tagline: z.string().trim().min(2).max(100), contactEmail: z.union([z.literal(""), z.email()]), instagramUrl: externalHref, facebookUrl: externalHref }),
  announcement: z.object({ enabled: z.boolean(), text: z.string().trim().max(140), href: internalHref }),
  seo: z.object({ title: z.string().trim().min(10).max(70), description: z.string().trim().min(30).max(170) }),
});
...
export const defaultSiteConfig: SiteConfig = {
  ...
  home: {
    eyebrow: "Hecho para Bella Vista", title: "Encontrá a alguien de confianza", highlight: "cerca tuyo.",
    ...
    featuredProfileSlug: "", heroAssetId: null, trustTitle: "Más claridad antes de llamar.",
  ...
  motion: defaultHomeMotion,
  footer: { tagline: "Hecho en Tucumán.", contactEmail: "", instagramUrl: "", facebookUrl: "" },
  announcement: { enabled: false, text: "", href: "" },
  seo: { title: "Listoficios | Profesionales en Bella Vista", description: "Encontrá profesionales y servicios en Bella Vista, Tucumán." },
};
