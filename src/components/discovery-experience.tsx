      ...
      <section data-motion-hero className="relative isolate overflow-hidden px-4 pb-12 pt-8 sm:px-6 sm:pb-16 sm:pt-10 lg:min-h-[760px] lg:px-8 lg:pb-24 lg:pt-14">
        ...
        <div className="mx-auto grid max-w-[1180px] items-center gap-12 lg:grid-cols-[1.08fr_.92fr]">
          <div className="max-w-[700px]" data-motion-hero-copy-group>
            ...
            {featured ? <div data-home-demo data-motion-hero-card aria-label="Ejemplo visual de un perfil profesional" className="mt-7 flex items-center gap-3 rounded-[22px] border border-[#d4e0dc] bg-white/90 p-3.5 shadow-lg lg:hidden"><Avatar initials={featured.initials} tone={featured.avatarTone} imageUrl={featured.avatarUrl} className="size-14" /><div className="min-w-0 flex-1"><p className="truncate font-semibold">{featured.name}</p><p className="truncate text-xs text-[var(--muted)]">{featured.trade} · Bella Vista</p><p className="mt-1 flex items-center gap-1 text-[11px] font-semibold text-[var(--brand)]"><Icon name="message" className="size-3.5" /> Contacto por WhatsApp</p></div><span className="rounded-full bg-[var(--paper)] px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[.08em] text-[var(--muted)]">Ejemplo</span></div> : null}
          </div>
          {featured ? <div data-home-demo data-motion-hero-card aria-label="Ejemplo visual de un perfil profesional" className="relative mx-auto hidden w-full max-w-[440px] lg:block"><div className="relative rounded-[38px] bg-[var(--ink)] p-7 text-white shadow-[0_35px_90px_rgba(19,55,47,.22)]"><div className="absolute right-8 top-8 size-28 rounded-full bg-[var(--accent)] opacity-80 blur-3xl" /><div className="relative"><div className="flex items-center gap-4"><Avatar initials={featured.initials} tone={featured.avatarTone} imageUrl={featured.avatarUrl} className="size-16" /><div className="min-w-0"><p className="truncate text-xl font-semibold">{featured.name}</p><p className="truncate text-sm text-white/60">{featured.trade} · Bella Vista</p></div><span className="ml-auto rounded-full border border-white/15 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[.08em] text-white/55">Ejemplo visual</span></div><div className="mt-7 grid grid-cols-2 gap-3"><div className="rounded-2xl bg-white/10 p-4"><Icon name="message" className="size-5 text-[var(--accent)]" /><p className="mt-3 text-sm font-semibold">Responde por WhatsApp</p></div><div className="rounded-2xl bg-white/10 p-4"><Icon name="check" className="size-5 text-[var(--accent)]" /><p className="mt-3 text-sm font-semibold">Servicio disponible</p></div></div><div className="mt-5 block rounded-2xl bg-white p-5 text-[var(--ink)]"><p className="text-xs font-medium text-[var(--muted)]">Así puede verse un servicio publicado</p><p className="mt-1 line-clamp-2 font-semibold">{featured.skills[0] || featured.trade}</p><span className="mt-4 flex items-center justify-between text-xs text-[var(--muted)]">Información organizada <Icon name="check" className="size-4 text-[var(--brand)]" /></span></div></div></div></div> : null}
        </div>
      </section>
