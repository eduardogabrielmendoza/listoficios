...
function iconForTrade(trade: string): IconName {
  const normalized = trade.toLocaleLowerCase("es");
  ...
  if (normalized.includes("carpint")) return "hammer";
  if (normalized.includes("jardin")) return "leaf";
  if (normalized.includes("refriger")) return "snowflake";
  return "wrench";
}
...
export function DiscoveryExperience({ initialProfessionals }: { initialProfessionals: ServiceProfile[] }) {
  ...
  const examples = useMemo(() => {
    const configured = site.home.featuredProfileSlug
      ? initialProfessionals.find((item) => item.slug === site.home.featuredProfileSlug)
      : undefined;
    return configured
      ? [configured, ...initialProfessionals.filter((item) => item.id !== configured.id)]
      : initialProfessionals;
  }, [initialProfessionals, site.home.featuredProfileSlug]);
  const [activeExample, setActiveExample] = useState(0);
  const featured = examples[activeExample % Math.max(examples.length, 1)];
  const featuredIcon = featured ? iconForTrade(featured.trade) : "wrench";

  useEffect(() => {
    if (examples.length < 2 || window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;
    let timer: ReturnType<typeof setInterval> | undefined;
    const start = () => {
      if (timer || document.hidden) return;
      timer = setInterval(() => setActiveExample((current) => (current + 1) % examples.length), 4200);
    };
    const stop = () => {
      if (!timer) return;
      clearInterval(timer);
    ...
    <section data-motion-hero className="relative isolate overflow-hidden px-4 pb-20 pt-12 sm:px-6 sm:pb-24 sm:pt-16 lg:min-h-[760px] lg:px-8 lg:pb-28 lg:pt-20">
      ...
      <div className="mx-auto grid max-w-[1180px] items-center gap-12 lg:grid-cols-[1.08fr_.92fr]">
        <div className="max-w-[700px]" data-motion-hero-copy-group>
          ...
          {featured ? <div data-home-demo data-motion-hero-card data-hero-service-compact className="hero-service-summary mt-7 flex lg:hidden"><div key={featured.id} className="hero-service-swap flex min-w-0 flex-1 items-center gap-3"><span className="hero-service-icon"><Icon name={featuredIcon} className="size-5" /></span><div className="min-w-0 flex-1"><p className="truncate font-semibold">{featured.trade}</p><p className="truncate text-xs text-[var(--muted)]">Servicio disponible en Bella Vista</p><p className="mt-1 flex items-center gap-1 text-[11px] font-semibold text-[var(--brand)]"><Icon name="message" className="size-3.5" /> Contacto directo</p></div></div></div> : null}
        </div>

        {featured ? <div data-home-demo data-motion-hero-card data-hero-service-card className="relative mx-auto hidden w-full max-w-[440px] lg:block"><div className="relative min-h-[364px] overflow-hidden rounded-[38px] bg-[var(--ink)] p-7 text-white shadow-[0_35px_90px_rgba(19,55,47,.22)]"><div className="absolute right-8 top-8 size-28 rounded-full bg-[var(--accent)] opacity-80 blur-3xl" /><div key={featured.id} className="hero-service-swap relative"><div className="flex items-center gap-4"><span className="hero-service-icon hero-service-icon-dark"><Icon name={featuredIcon} className="size-6" /></span><div className="min-w-0"><p className="truncate text-xl font-semibold">{featured.trade}</p><p className="truncate text-sm text-white/60">Servicio local · Bella Vista</p></div></div><div className="mt-7 grid grid-cols-2 gap-3"><div className="rounded-2xl bg-white/10 p-4"><Icon name="message" className="size-5 text-[var(--accent)]" /><p className="mt-3 text-sm font-semibold">Contacto directo</p></div><div className="rounded-2xl bg-white/10 p-4"><Icon name="check" className="size-5 text-[var(--accent)]" /><p className="mt-3 text-sm font-semibold">Información ordenada</p></div></div><div className="mt-5 block min-h-[116px] rounded-2xl bg-white p-5 text-[var(--ink)]"><p className="text-xs font-medium text-[var(--muted)]">Servicio publicado</p><p className="mt-1 line-clamp-2 font-semibold">{featured.skills[0] || featured.trade}</p><span className="mt-4 flex items-center justify-between text-xs text-[var(--muted)]">Zona, modalidad y precio <Icon name="check" className="size-4 text-[var(--brand)]" /></span></div></div></div></div> : null}
      </div>
    </section>
  );
}
