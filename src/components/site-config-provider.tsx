...
export function SiteConfigProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => { const controller = new AbortController(); void fetch("/api/v1/site-config", { signal: controller.signal }).then((response) => response.ok ? response.json() : null).then((payload) => { if (payload?.data) setConfig(normalizeSiteConfig(payload.data)); }).catch(() => undefined); return () => controller.abort(); }, []);
