  ...
  return <div className="mt-8 grid gap-6 xl:grid-cols-[1fr_380px]">
    <div className="rounded-[24px] border border-[var(--line)] bg-white p-5 sm:p-7">
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <Field label="Fragmento destacado" value={config.home.highlight} onChange={(value) => home("highlight", value)} />
        ...
        <div className="sm:col-span-2"><label className="flex items-start justify-between gap-4 rounded-[18px] bg-[var(--paper)] p-4"><span><strong className="block text-sm">Scroll suave global</strong><small className="mt-1 block text-xs leading-5 text-[var(--muted)]">Se aplica en escritorio a todas las páginas. Móvil y movimiento reducido conservan scroll nativo.</small></span><input type="checkbox" checked={config.motion.smoothScrollEnabled} onChange={(event) => motion("smoothScrollEnabled", event.target.checked)} className="mt-1 size-5 accent-[var(--brand)]" /></label></div>
        ...
        <div className="sm:col-span-2 mt-3 border-t border-[var(--line)] pt-6"><p className="section-kicker">Mapa de posibilidades</p></div>
        <Field label="Etiqueta de la sección" value={config.motion.servicesEyebrow} onChange={(value) => motion("servicesEyebrow", value)} /><Field label="Título de la sección" value={config.motion.servicesTitle} onChange={(value) => motion("servicesTitle", value)} />
        <Field label="Momento: necesidad" value={config.motion.servicesNeedText} onChange={(value) => motion("servicesNeedText", value)} /><Field label="Momento: oficios" value={config.motion.servicesCategoriesText} onChange={(value) => motion("servicesCategoriesText", value)} />
        <Field label="Momento: cobertura" value={config.motion.servicesZonesText} onChange={(value) => motion("servicesZonesText", value)} /><Field label="Momento: información" value={config.motion.servicesClarityText} onChange={(value) => motion("servicesClarityText", value)} />
    ...
    <aside className="rounded-[26px] bg-[var(--ink)] p-6 text-white xl:sticky xl:top-24 xl:self-start">
      <div className="flex items-center justify-between gap-3"><p className="text-xs font-semibold uppercase tracking-[.13em] text-[var(--lime)]">Vista previa</p><div className="flex rounded-full bg-white/10 p-1 text-[10px]"><button type="button" onClick={() => setPreviewMode("desktop")} className={`rounded-full px-2.5 py-1 ${previewMode === "desktop" ? "bg-white text-[var(--ink)]" : "text-white/65"}`}>Escritorio</button><button type="button" onClick={() => setPreviewMode("mobile")} className={`rounded-full px-2.5 py-1 ${previewMode === "mobile" ? "bg-white text-[var(--ink)]" : "text-white/65"}`}>Móvil</button></div></div>
      <div className={`mt-5 transition-[max-width] ${previewMode === "mobile" ? "max-w-[240px]" : "max-w-full"}`}><h2 className={`${previewMode === "mobile" ? "text-3xl" : "text-4xl"} font-semibold leading-[.98] tracking-[-.05em]`}>{config.home.title} <span className="text-[var(--lime)]">{config.home.highlight}</span></h2><p className="mt-4 text-sm leading-6 text-white/65">{config.home.description}</p><span className="mt-6 inline-flex rounded-full bg-[var(--lime)] px-5 py-3 text-sm font-semibold text-[var(--ink)]">{config.home.primaryCtaLabel}</span></div>
      <div className="mt-7 border-t border-white/10 pt-6"><p className="text-[10px] font-semibold uppercase tracking-[.15em] text-[var(--lime)]">Narrativa {config.motion.enabled ? "activa" : "desactivada"} · Scroll {config.motion.smoothScrollEnabled ? "suave" : "nativo"}</p><p className="mt-3 text-lg font-semibold">{config.motion.needText}</p><p className="mt-2 text-sm text-white/55">“{config.motion.searchText}”</p><p className="mt-5 text-[10px] font-semibold uppercase tracking-[.15em] text-[var(--lime)]">{config.motion.servicesEyebrow}</p><p className="mt-2 text-sm font-semibold">{config.motion.servicesTitle}</p><p className="mt-3 text-xs leading-5 text-white/45">{config.motion.servicesClarityText}</p></div>
    </aside>
  </div>;
}
